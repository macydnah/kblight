...
void writing() { if((fp = fopen(SYSPATH,"w")) == NULL) { fprintf(stderr, "Error! writing file.\n"); exit(1); } }
...
int main(int argc, char *argv[])
{
	...
	if((fp = fopen(SYSPATH,"r")) == NULL) { fprintf(stderr, "Error! reading file.\n"); exit(1); }
	fscanf(fp, "%hd", &mode); fclose(fp);
